...
public class Query {
	...
	public static enum ResultFormat {
		...
		ATOM("atom"), RSS("rss"), JSON("json"), JSONC("jsonc"), ATOM_IN_SCRIPT("atom-in-script"), RSS_IN_SCRIPT(
				"rss-in-script"), JSON_IN_SCRIPT("json-in-script"), JSONC_IN_SCRIPT("jsonc-in-script"), JSON_XD(
				"json-xd"), ATOM_SERVICE("atom-service");
	...
	/**
	 * The maximum number of results to return for the query. A value of {@link #UNDEFINED} indicates the server can
	 * determine the maximum size.
	 */
	private int maxResults = UNDEFINED;

	/**
	 * The expected result format for the query. The default is {@link ResultFormat#DEFAULT}.
	 */
	private ResultFormat resultFormat = ResultFormat.DEFAULT;

	/**
	 * The strictness of the query parameter parsing on the server. If strict mode is enabled any unknown query
	 * parameters will be rejected. The default value is false.
	 */
	...
	/**
	 * Constructs a new Query object that targets a feed. The initial state of the query contains no parameters, meaning
	 * all entries in the feed would be returned if the query was executed immediately after construction.
	 * 
	 * @param feedUrl
	 *            the URL of the feed against which queries will be executed.
	 */
	public Query(String feedUrl) {
	 ...
	 * 
	 * @param query
	 *            the full text search query string. A value of {@code null} disables full text search for this Query.
	 */
	 ...
	 * match.
	 * <p>
	 * The CategoryFilter can contain multiple category criteria (inclusive or exclusive). If it does contain multiple
	 * categories, then the query matches if any one of the category filter criteria is met, i.e. it is a logical 'OR'
	 * of the contained category criteria. To match, an entry must contain at least one included category or must not
	 * contain at least one excluded category.
	 * <p>
	 * It is also possible to add multiple CategoryFilters to a Query. In this case, each individual CategoryFilter must
	 * be true for an entry to match, i.e. it is a logical 'AND' of all CategoryFilters.
	 * 
	...
	public static class CategoryFilter {
		...
		/**
		 * Creates a new category filter using the supplied inclusion and exclusion lists. A null value for either is
		...
		/**
		 * Creates a simple category filter containing only a single {@link ICategory}.
		 * 
		 * @param category
		 *            an initial category to add to the filter.
		 */
		...
		/**
		 * Adds a new {@link ICategory} to the query, indicating that entries containing the category should be
		 * considered to match.
		 * 
		 * @param category
		 *            the category to add to query parameters.
		 */
		...
		/**
		 * Adds a new {@link ICategory} to the query, indicating that entries that do not contain the category should be
		 * considered to match.
		 * 
		 * @param category
		 *            the category to add to query parameters.
		...
		/**
		 * Returns a string representation for the category conditions in the CategoryFilter, in the format used by a
		 * Query URI.
		 */
		@Override
	...
	/**
	 * Adds a new CategoryFilter to the query. For an entry to match the query criteria, it must match against
	 * <b>all</b> CategoryFilters that have been associated with the query.
	 */
	...
	/**
	 * Sets the author name or email address used for the query. Only entries with an author whose name or email address
	 * match the specified value will be returned.
	 * 
	 * @param author
	 *            the name or email address for matched entries. A value of {@code null} disables author-based matching.
	 */
	...
	/**
	 * Returns the author name or email address used for the query. Only entries with an author whose name or email
	 * address match the specified value will be returned.
	 * 
	 * @return the name or email address for matched entries. A value of {@code null} means no author-based matching.
	 */
	...
	/**
	 * Sets the minimum updated timestamp used for the query. Only entries with an updated timestamp equal to or later
	 * than the specified timestamp will be returned.
	 * 
	 * @param updatedMin
	...
	/**
	 * Returns the minimum updated timestamp used for this query. Only entries with an updated timestamp equal to or
	 * later than the specified timestamp will be returned.
	 * 
	...
	/**
	 * Sets the maximum updated timestamp used for the query. Only entries with an updated timestamp earlier than the
	...
	/**
	 * Returns the maximum updated timestamp used for this query. Only entries with an updated timestamp earlier than
	 * the specified timestamp will be returned.
	 * 
	...
	/**
	 * Sets the minimum published timestamp used for the query. Only entries with a published time equal to or later
	 * than the specified timestamp will be returned.
	 * 
	 * @param publishedMin
	...
	/**
	 * Returns the minimum published timestamp used for this query. Only entries with a published time equal to or later
	 * than the specified timestamp will be returned.
	 * 
	...
	/**
	 * Sets the maximum published timestamp used for the query. Only entries with a published time earlier than the
	 * specified timestamp will be returned.
	 * 
	 * @param publishedMax
	...
	/**
	 * Returns the maximum published timestamp used for this query. Only entries with a published timestamp earlier than
	 * the specified timestamp will be returned.
	 * 
	...
	/**
	 * Returns the current start index value for the query, or {@link #UNDEFINED} if start index has not been set.
	 */
	...
	/**
	 * Sets the maximum number of results to return for the query. Note: a GData server may choose to provide fewer
	 * results, but will never provide more than the requested maximum.
	 * 
	 * @param maxResults
	 *            the maximum number of results to return for the query. A value of zero indicates that the server is
	 *            free to determine the maximum value.
	 * @throws IllegalArgumentException
	...
	/**
	 * Returns the maximum number of results to return for the query, or {@link #UNDEFINED} if max results has not been
	 * set.
	 * <p>
	 ...
	 * 
	 * @param resultFormat
	 *            ResultFormat value indicating the desired format.
	 */
	 ...
	 * 
	 * @param strict
	 *            true if strict parsing should be enabled for this query.
	 */
	 ...
	 * 
	 * @return all custom parameters for the query. An empty list will be returned if there are no custom parameters.
	 */
	 ...
	 * @return all parameters that have the specified name. An empty list will be returned if there are no matching
	 *         parameters.
	 */
	...
	/**
	 * Returns the relative query URI that represents only the query parameters without any components related to the
	 * target feed. Subclasses of the Query class may override this method to add additional URI path elements or HTTP
	 * query parameters to represent service-specific parameters.
	 * 
	...
	public String getQueryUri() {
		...
		if (queryString != null) {
			appendQueryParameter(queryBuf, GDataProtocol.Query.FULL_TEXT, CharEscapers.uriEscaper().escape(queryString));
		}

		if (author != null) {
			appendQueryParameter(queryBuf, GDataProtocol.Query.AUTHOR, CharEscapers.uriEscaper().escape(author));
		}

		if (resultFormat != ResultFormat.DEFAULT) {
			appendQueryParameter(queryBuf, GDataProtocol.Parameter.ALT, resultFormat.paramValue());
		}

		if (updatedMin != null) {
			appendQueryParameter(queryBuf, GDataProtocol.Query.UPDATED_MIN, CharEscapers.uriEscaper().escape(
					updatedMin.toString()));
		}

		if (updatedMax != null) {
			appendQueryParameter(queryBuf, GDataProtocol.Query.UPDATED_MAX, CharEscapers.uriEscaper().escape(
					updatedMax.toString()));
		}

		if (publishedMin != null) {
			appendQueryParameter(queryBuf, GDataProtocol.Query.PUBLISHED_MIN, CharEscapers.uriEscaper().escape(
					publishedMin.toString()));
		}

		if (publishedMax != null) {
			appendQueryParameter(queryBuf, GDataProtocol.Query.PUBLISHED_MAX, CharEscapers.uriEscaper().escape(
					publishedMax.toString()));
		}

		if (startIndex != UNDEFINED) {
			appendQueryParameter(queryBuf, GDataProtocol.Query.START_INDEX, Integer.toString(startIndex));
		}

		if (maxResults != UNDEFINED) {
			appendQueryParameter(queryBuf, GDataProtocol.Query.MAX_RESULTS, Integer.toString(maxResults));
		}

		if (fields != null) {
			appendQueryParameter(queryBuf, GDataProtocol.Query.FIELDS, CharEscapers.uriEscaper().escape(fields));
		}
		...
		for (CustomParameter customParameter : customParameters) {
			appendQueryParameter(queryBuf, CharEscapers.uriEscaper().escape(customParameter.name), CharEscapers
					.uriEscaper().escape(customParameter.value));
		}

		if (callback) {
			appendQueryParameter(queryBuf, GDataProtocol.Parameter.CALLBACK, "");
		}

		return pathBuf.toString() + queryBuf.toString();
	}
	...
	/**
	 * Gets an existing Integer custom paramter, with null signifying that the parameter is not specified or not an
	 * integer.
	 * 
	 * @param name
	 *            the name of the parameter
	 * @return the value of the parameter, or null if it is unspecified or non-integer
	 */

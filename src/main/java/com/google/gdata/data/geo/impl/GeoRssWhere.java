...
/**
 * The georss:where element. This can contain other elements, so it is extensible, but it also acts as a Point object.
 * It can also act as a Box as well, and can be extended to support other GML object types.
 * 
 * 
 */
@JsonObject
public class GeoRssWhere extends ExtensionPoint implements Point, Box {
	...
	/**
	 * Constructs a georss:where element containing the given lat/lon point. If the given lat and lon are null, will
	 * construct an empty gml:Point element.
	 */
	...
	/**
	 * Constructs a georss:where element containing the given point. If the point is already a gml:Point object, it will
	 * be used as the extension directly, otherwise a copy will be created. If the point is null no extensions will be
	 * added.
	 */
	...
	/**
	 * Constructs a georss:where element containing the four coordinates that define a box. If all of the points are
	 * null then an empty gml:envelope will be created.
	 */
	...
	/**
	 * Constructs a georss:where element containing the given gml:lowerCorner and gml:upperCorner elements. If both
	 * lower and upper are null then an empty gml:envelope will be created.
	 */
	...
	/*
	 * Set the latitude and longitude by creating or removing the gml:Point element as needed. If the existing wrapped
	 * point is non-null but the given lat and lon are both null, it is removed. Otherwise its new latitude and
	 * longitude are set. If the gml:Point element doesn't exist, it is only created if the given coordinates are
	 * non-null. We also rely on the gml:Point object to do checking of valid values.
	 */
	...
	/*
	 * Set the bounding box points. This correctly handles all cases where the existing gml:envelope is null or has null
	 * values. In particular:
	 * 
	 * 1) Remove the lower and upper corner of the GmlEnvelope if lowerLeft and upperRight are null. 2) Create a new
	 * extension if it doesn't exist and either lowerLeft or upperRight are non-null. 3) Otherwise just set the
	...
	public static ExtensionDescription getDefaultDescription(boolean repeatable) {
		...
		desc.setPointName(Namespaces.GEO_RSS_NAMESPACE + "$" + NAME);
	...
	/*
	 * Declare the extensions for georss:where. This contains elements for points, boxes, polygons, and lines. This can
	 * be extended to include other types of information as well.
	 */
	@Override
